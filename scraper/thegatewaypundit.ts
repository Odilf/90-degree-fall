...
async function scrape(month: number, day: number) {
	...
	await Promise.all([
		...
		await page.evaluate(() => {
			const as = document.querySelectorAll('a.post-link') as NodeListOf<HTMLAnchorElement>;
	...
	const article_text = await page.evaluate(() => {
		const div = document.querySelector('.entry-content') as HTMLElement
